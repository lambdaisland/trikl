  ...
  (let [buf-size 8192
    ...
    (fn []
      (let [end (.read in buffer 0 buf-size)
            {:keys [dest-end commands]} (telnet/filter-commands {:src buffer :dest buffer' :end end})]
        [commands (.decode decoder (ByteBuffer/wrap buffer' 0 dest-end))]))))
...
(defn input-consumer-loop
  ...
  (let [read-input-stream (char-telnet-reader in)]
    (try
      (loop [state {}]
        #_(prn state)
        (let [{:keys [^CharBuffer char-buf]} state]
          (if (or (nil? char-buf) (not (.hasRemaining char-buf)))
            ;; Fill byte buffer, pull out telnet codes, decode what's left to char buffer
            ...
            (let [ch (char (.get char-buf))
              ...
              (cond
                ...
                ;;
                ;; We have a conundrum here, since we can't readily
                ;; differentiate an ESC coming from keyboard input, vs an ESC
                ;; which is the start of an escape sequence generated by the
                ;; terminal emulator. We could look at the next character to see
                ;; if it looks like an escape sequence, but only if the next
                ;; character is already buffered.
                ;;
                ;; So if we are unsure we still initiate the escape sequence
                ;; handling. If we're unsure if more is coming then we also
                ;; produce an :escape keyboard event immediately. If we have
                ;; more characters buffered we continue processing, but generate
                ;; an escape keyboard event if it turns out what follows is not
                ...
                (if (char-in-range? \@ ch \~)
                  ...
                  (do
                    (when-not (:esc-sent? state)
                      (handler {:type :input :char (char ESC)}))
                    (doseq [ch (:ansi-command state)]
                      (handler {:type :input :char ch}))
                    (handler {:type :input :char ch})
                    (recur (dissoc state :ansi-state :ansi-command :esc-sent?))))
